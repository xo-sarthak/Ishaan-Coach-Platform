...
export async function fetchYouTubeShorts(): Promise<YoutubeVideo[]> {
  ...
  try {
    ...
    // 3. Map to our format
    // YouTube doesn't have a direct "Shorts" flag in the playlistItems, 
    // but Shorts are just videos. We'll fetch them all and filter or just show the latest.
    // In a more advanced version, we'd check the video duration via videos.list.
    return videosData.items.map((item: any) => ({
      id: item.contentDetails.videoId,
      title: item.snippet.title,
      thumbnail: item.snippet.thumbnails.high?.url || item.snippet.thumbnails.default?.url,
      url: `https://www.youtube.com/shorts/${item.contentDetails.videoId}`, // Forcing shorts URL format
