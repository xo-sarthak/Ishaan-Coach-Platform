      ...
      <div className="max-w-5xl mx-auto px-6 mt-16 md:mt-24">
        <div className="bg-card border border-border/50 rounded-[2.5rem] p-8 md:p-14 shadow-sm overflow-hidden relative">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 mb-12 pb-10 border-b border-border/40 relative z-10">
            ...
            {/* Toggle Switch - Interactive & Clean */}
            <div className="relative inline-flex h-14 items-center justify-center rounded-2xl bg-muted/60 p-1.5 w-full lg:w-auto shrink-0 border border-border/50">
              <button
                onClick={() => setCoachType("relationship")}
                className={`relative z-10 inline-flex h-full items-center justify-center whitespace-nowrap rounded-xl px-8 text-xs font-bold uppercase tracking-wider transition-colors w-1/2 lg:w-auto ${coachType === "relationship" ? "text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                  }`}
              >
                ...
                <span className="relative z-20">Relationships</span>
              </button>
              <button
                onClick={() => setCoachType("life")}
                className={`relative z-10 inline-flex h-full items-center justify-center whitespace-nowrap rounded-xl px-8 text-xs font-bold uppercase tracking-wider transition-colors w-1/2 lg:w-auto ${coachType === "life" ? "text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                  }`}
              >
                {coachType === "life" && (
                  <motion.div
                ...
                <span className="relative z-20">Life & Performance</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-background/40 dark:bg-muted/10 rounded-3xl md:p-6 border border-border/30 backdrop-blur-sm relative z-10">
            <ChatInterface
              type={coachType}
              title={coachType === "relationship" ? "Relationship Strategy" : "Life Strategy"}
