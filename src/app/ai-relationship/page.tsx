import ChatInterface from "@/components/ChatInterface";
import Link from "next/link";

export const metadata = {
  title: "AI Relationship Coach | Platforms",
  description: "Chat with your AI relationship coach.",
};

export default function AiRelationshipPage() {
  return (
    <div className="w-full max-w-5xl mx-auto px-6 py-12">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
        <div className="max-w-xl">
          <h1 className="text-4xl font-bold tracking-tight mb-4">Relationship Coach</h1>
          <p className="text-lg text-muted-foreground">
            Navigate dating, breakups, and emotional clarity with pragmatic, empathetic advice. I am trained on the creator's best relationship frameworks.
          </p>
        </div>
        
        {/* Toggle Switch */}
        <div className="inline-flex h-11 items-center justify-center rounded-xl bg-muted p-1 text-muted-foreground w-full md:w-auto shrink-0 border border-border/50">
          <div className="inline-flex h-full items-center justify-center whitespace-nowrap rounded-lg px-6 text-sm font-semibold transition-all bg-background text-foreground shadow-sm w-1/2 md:w-auto">
            Relationship
          </div>
          <Link 
            href="/ai-life" 
            className="inline-flex h-full items-center justify-center whitespace-nowrap rounded-lg px-6 text-sm font-medium transition-all hover:text-foreground hover:bg-muted/80 w-1/2 md:w-auto"
          >
            Life Coach
          </Link>
        </div>
      </div>
      
      <ChatInterface 
        type="relationship" 
        title="Relationship Chat" 
        description="Ask me about texting anxiety, dealing with an ex, or how to communicate your needs."
      />
    </div>
  );
}
